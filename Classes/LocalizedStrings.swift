...
public struct LocalizedStrings: Codable, ExpressibleByArrayLiteral, ExpressibleByDictionaryLiteral {
        ...
        for (langPrefix, text) in ingredientsMap {
            if let language = Language.allCases.first(where: { $0.prefix == langPrefix }) {
                tempIngredients.append(LocalizedString(text, language: language))
                 ...
                 print("Warning: Unknown language prefix '\(langPrefix)' found during decoding LocalizedStringMap.")
            }
        }
    ...
    public func encode(to encoder: Encoder) throws {
        if localizations.isEmpty {
            ...
            let ingredientsMap = localizations.reduce(into: [String: String]()) { result, localizedString in
    ...
    public subscript(language: Language) -> String? {
        ...
        set {
            ...
            localizations.removeAll { $0.language == language }
            // Add new value if not nil
